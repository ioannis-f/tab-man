...
document.addEventListener("drop", function(event) {
  ...
  let srcPos =  b.findPosById(dragged.id.split("_")[0]);
  let dstPos =  b.findPosById(event.target.id.split("_")[0]);

  let tmp = tm.list[srcPos];
  tm.list.splice(srcPos, 1);           // Remove from source position
  if(dstPos==""){
    log("### empty");
    dstPos = tm.list.length;
  }
  tm.list.splice(dstPos, 0, tmp);      // Insert to destination position
  log(tm);
//  b.saveToStorage(); // Save to storage
  createview();
